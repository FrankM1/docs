...
class Tutorial extends React.Component {
    componentDidMount() {
        ...
        // The other option is to create a plugin for WordpressPosts.
        Prism.highlightAll()
    }

    render() {
        const post = this.props.data.wordpressWpTutorials
        ...
        return (
            <>
                <MetaData
                    ...
                    // with the feature image that is set in the Wordpress post
                ...
                <Layout headerDividerStyle="shadow" mainClass="bg-white">
                    ...
                    <div className={`${Spirit.page.xl} pb15`}>
                        <article className="mw-content center pa7 pa15-ns pb10">
                            ...
                            <section className="post-content tutorial-content external-scripts" dangerouslySetInnerHTML={{ __html: post.content }} />
                        </article>
                    </div>
                </Layout>
            </>
...
Tutorial.propTypes = {
    data: PropTypes.shape({
        wordpressWpTutorials: PropTypes.shape({
...
export const articleQuery = graphql`
    query($slug: String!) {
        ...
        wordpressWpTutorials(slug: { eq: $slug }) {
            title
            content
        }
    }
